...
def get_all_structures() -> Sequence[sa.Row[Tuple[Structure]]]:
    ...
    if st.session_state.super:
        ...
        stmt = sa.select(Structure).where(Structure.structure_id != super_shipyard_id).filter(Structure.structure_type_id.in_(valid_structures))
...
def main():
    ...
    if st.session_state.cost_results is not None and st.session_state.selected_item_for_display == selected_item:
        ...
        if selected_structure_for_materials:
            ...
            if calculate_clicked:
                job = JobQuery(item=selected_item, item_id=type_id, group_id=group_id, runs=runs, me=me, te=te, material_prices=st.session_state.price_source)
